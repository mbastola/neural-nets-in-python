...
class CNN3D(object):
    ...
    def build(self, initial_shape, debug):
        N, width, height, depth, ch = initial_shape
        mi = ch
        outw = width
        outh = height
        outd = depth
        ...
        for layer_param in self.architecture_params:
            ...
            if layer_type == 'C':
                ...
                outh =math.ceil(outh/layer_param['stride'][1])
                outd =math.ceil(outd/layer_param['stride'][2])
                layer_param['num_input'] = mi
                layer = Conv3DLayer(layer_param)
                cp_first = False
    ...
    def fit(self, X, Y, batch_sz=500, epochs=1, method="adam",debug=False):
        ...
        #X, Y = X[:-1000], Y[:-1000]
        Xvalid, Yvalid = X[-batch_sz:], Y[-batch_sz:]
        X, Y = X[:-batch_sz], Y[:-batch_sz]
        
        Yvalid_flat = np.argmax(Yvalid, axis=1) # for calculating error rate
        
        # initialize the architecture
        self.params = self.build(X.shape, debug)
        N, width, height, depth, ch = X.shape
        
        # set up tensorflow functions and variables
        tfX = tf.placeholder(tf.float32, shape=(None, width, height, depth, ch), name='X')
    ...
    def predict(self, X, batch_sz = None):
        if (str(type(X)) == "<class 'numpy.ndarray'>"):
            ...
            with tf.Session() as session:
                ...
                N, width, height, depth, c = X.shape
                if batch_sz == None:
                    batch_sz = N

                tfX = tf.placeholder(tf.float32, shape=(None, width, height, depth, c), name='X')
                ...
                for j in range(n_batches+1):
                    ...
                    pbatch = session.run(prediction, feed_dict={tfX: Xbatch})
                
                    print(set(pbatch), len(set(pbatch)))
                    print(p.shape)
                    p = np.concatenate([p,pbatch])
                return p
        else:
            pY = self.forward(X)
