...
class Conv3DLayer(object):
    ...
    def __init__(self, param):
        ...
        fw, fh, fd = self.ksize
        tag = self.layer_type + str(self.layer_depth)
        self.W = tf.Variable(tf.truncated_normal([fw, fh,fd, mi, mo], stddev=0.03), name="W"+tag)
        self.b = tf.Variable(tf.constant(0.1, shape=[mo]), name="b"+tag)
    ...
    def forward(self, X):
        ...
        conv_out = tf.nn.conv3d(X, self.W, strides=[1, sw, sh, sd, 1], padding='SAME')
...
class Pool3DLayer(object):
    ...
    def forward(self, X):
        ...
        pool_out = None
        if self.pool == 'max':
            pool_out = tf.nn.max_pool3d(
                X,
                ksize=[1, fw, fh, fd, 1],
                strides=[1, sw, sh, sd, 1],
                padding='SAME'
            )
        elif self.pool == 'avg':
            pool_out = tf.nn.avg_pool3d(
                X,
                ksize=[1, fw, fh, fd, 1],
                strides=[1, sw, sh, sd, 1],
                padding='SAME'
